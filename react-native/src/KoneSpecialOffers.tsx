...
const API_ENDPOINT = 'https://go.kone.vc/mcp/sales';
...
async function sendToKone(
  ...
  const payload: Record<string, string> = { prompt, api_key: apiKey };
...
export function KoneSpecialOffers({
  ...
  const doSend = useCallback(async (text: string) => {
    ...
    setLoading(true);
    try {
      const result = await sendToKone(apiKey, prompt, responseIdRef.current);
